...
export default function PropertyMapInner({
  ...
  const [coordinates, setCoordinates] = useState<Coordinates>(() => {
    ...
    if (latitude !== undefined && longitude !== undefined) {
      // console.log(`PropertyParams - MapInner: Using provided coordinates: ${latitude}, ${longitude}`);
      return { latitude, longitude };
    }
    // console.log('PropertyParams - MapInner: Using default coordinates initially');
  ...
  shadowUrl: require("leaflet/dist/images/marker-shadow.png").default,
        ...
        console.warn("PropertyParams - MapInner: Failed to load marker icons, using defaults:", error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const extractCoordinates = async () => {
      if (mapLink) {
        // console.log(`PropertyParams - MapInner: Map link provided: ${mapLink}`);
        setLoading(true);
        try {
          const coords = await getCoordinatesFromMapLink(mapLink);
          // console.log(`PropertyParams - MapInner: Final coordinates to be used: ${coords.latitude}, ${coords.longitude}`);
          setCoordinates(coords);
        } catch (error) {
          console.error('PropertyParams - MapInner: Failed to extract coordinates from mapLink:', error);
          console.error('PropertyParams - MapInner: Map link that failed:', mapLink);
        ...
        // console.log('PropertyParams - MapInner: Using default coordinates');
        setCoordinates(getDefaultCoordinates());
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    // Prevent multiple maps on re-render - UNIQUE ID for main property
    const mapId = "mainPropertyMap";
    const existingMap = L.DomUtil.get(mapId);
    if (existingMap && (existingMap as any)._leaflet_id) {
      (existingMap as any)._leaflet_id = null;
    }

    // console.log(`PropertyParams - MapInner: Initializing map at final coordinates: ${coordinates.latitude}, ${coordinates.longitude}`);
    
    // Initialize map with extracted coordinates
    const map = L.map(mapId).setView([coordinates.latitude, coordinates.longitude], 15);
    setMapInstance(map);
