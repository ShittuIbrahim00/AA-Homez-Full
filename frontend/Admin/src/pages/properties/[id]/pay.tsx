...
const PropertyPaymentPage = () => {
  ...
  const { property, loading: propertyLoading, error: propertyError } = useFetchProperty(id as string);
  ...
  // Pre-fill amount with property price when property loads
  useEffect(() => {
    if (property && property.price) {
      const priceNumber = parsePriceString(property.price);
      setPayData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      toastError("Property ID is missing.");
      return;
    }
    ...
    try {
      const result = await payForProperty(Number(id), {
        aid: payData.aid,
      ...
      if (result.success) {
        ...
        // Navigate back to the property page to refresh all data
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      <BackButton onClick={() => router.push(`/properties/${id}`)} />
      
      <div className="mt-6 bg-white rounded-xl shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Pay for Property</h1>
        <p className="text-gray-600 mb-6">Assign this property to an agent and record payment details</p>
        
        {/* Property Summary */}
        <div className="bg-gray-50 rounded-lg p-4 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="font-semibold text-lg text-gray-900">{property.name}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Pre-filled with property price: {formatPrice(property.price)}
            </p>
          </div>
...
export default PropertyPaymentPage;