...
export type ScheduleVistSubProperty = {
  sid: number;
  date: string;
  time: string;
  title: string;
...
export default async function handler(
  ...
  const { clientName, clientPhone, date, end, sid, start, time, title } =
    req.body as ScheduleVistSubProperty;
  const token = req.headers["guard-token"];

  try {
    const response = await fetch(
      " https://aa-homez.onrender.com/api/v1/property/sub/schedule",
      {
        method: "POST",
        headers: {
